...
#	HEADERIZE='/^Filesystem/ {print header; next}'
	...
	NORMALIZE='{sub("^/dev/", "", $1); sub("s[0-9]+$", "", $1)} function fromKB(KB) {MB = KB/1024; if (MB<1024) return MB "M"; GB = MB/1024; return GB "G"} {$3=fromKB($3); $4=fromKB($4); $5=fromKB($5)}'
...
$CMD | tee $TEE_DEST | $AWK "$FILTER_PRE $MAP_FS_TO_TYPE $HEADERIZE $FORMAT $FILTER_POST $NORMALIZE $PRINTF"
